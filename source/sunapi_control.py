...
class CameraControl:
    ...
    def __init__(self, ip, user, password):
        logger.debug("Init camera control for addr=%r user=%r", ip, user)
        self.__cam_ip = ip
    ...
    def _camera_command(self, value_cgi, payload: dict):
        ...
        """
        logger.debug("camera_command(%s)", payload)

        url = "http://" + self.__cam_ip + "/stw-cgi/" + value_cgi

        resp = requests.get(
            url,
            auth=HTTPDigestAuth(self.__cam_user, self.__cam_password),
            params=payload,
        )
    ...
    def stop_control(self):
        ...
        resp = self._camera_command(
            "ptzcontrol.cgi",
            {"msubmenu": "stop", "action": "control", "OperationType": "All"},
        )
    ...
    def absolute_control(
        ...
        init_pos, initial_zoom_pulse = (
            self.operation_finished()
        )  # takes current (pan, tilt, zoom) values as an array
        ...
        if zoom_pulse is None:
            ...
            while abs(current_position - finished_position) > error_margin:
                ...
                final_position = (
                    current_position  # final_position = last position recorded
                )

                init_pos, initial_zoom_pulse = (
                    self.operation_finished()
                )  # update current position array and zoom_pulse

                current_position = np.sum(init_pos)  # update current position

                if (
                    final_position == current_position
                ):  # if the final position and current position are the same
                    i = i + 1  # add one to counter

                if (
                    i == 5
                ):  # if the current_position and final_position have been the same for five times in a row
            ...
            while abs(current_zoom_pulse - finished_zoom_pulse) > 5:
                ...
                final_zoom_pulse = (
                    current_zoom_pulse  # final_zoom_pulse = last zoom pulse recorded
                )
                ...
                if (
                    final_zoom_pulse == current_zoom_pulse
                ):  # if the final_zoom_pulse and current_zoom_pulse are the same
                    i = i + 1  # add one to counter

                if (
                    i == 5
                ):  # if the current_zoom_pulse and final_zoom_pulse have been the same for five times in a row
                    logger.error("end of command error")
                    break  # break the loop
        ...
        logger.debug("elapsed_time: " + str(elapsed_time))
    ...
    def relative_control(
        ...
        init_pos, initial_zoom_pulse = (
            self.operation_finished()
        )  # takes current position values as an array

        current_position = np.sum(
            init_pos
        )  # sums elements in the initial position array
        ...
        current_zoom_pulse = (
            initial_zoom_pulse  # provides the absolute zoom_pulse position
        )
        ...
        if (
            current_pan + pan == 360
        ):  # pan at 360 will be set to zero. Remove pan from finished_position
    ...
    def continuous_control(
        ...
        if focus not in ("Near", "Far", "Stop", None):
            raise Exception(
                "Unauthorized command: Please enter a string from the choices: 'Near', 'Far', or 'Stop'"
            )
        ...
        return self._camera_command(
            "ptzcontrol.cgi",
            {
                "msubmenu": "continuous",
                "action": "control",
                "NormalizedSpeed": normalized_speed,
                "Pan": pan,
                "Tilt": tilt,
                "Zoom": zoom,
                "Focus": focus,
    ...
    def moving_to_home_position(self, channel: int = None):
        ...
        return self._camera_command(
            "ptzcontrol.cgi",
            {"msubmenu": "home", "action": "control", "Channel": channel},
        )
    ...
    def zoom_out(self):
        ...
        resp = self._camera_command(
            "ptzcontrol.cgi",
            {"msubmenu": "areazoom", "action": "control", "Type": "1x"},
        )
    ...
    def aux_control(self, command: str = None):
        ...
        return self._camera_command(
            "ptzcontrol.cgi",
            {"msubmenu": "aux", "action": "control", "Command": command},
        )
    ...
    def group_control(self, channel: int = None, group: int = None, mode: str = None):
        ...
        if mode not in ("Start", "Stop", None):
            raise Exception(
                "Unauthorized command: Please enter a string from the choices: 'Start' or 'Stop'"
            )
    ...
    def applications(self):
        ...
        resp = self._camera_command(
            "opensdk.cgi", {"msubmenu": "apps", "action": "view"}
        )
    ...
    def snap_shot(self, directory: str = None):
        ...
        resp = self._camera_command(
            "video.cgi", {"msubmenu": "snapshot", "action": "view"}
        )
        ...
        if directory is None:  # if no directory is passed
            ...
            string = a1.replace(
                "/source", "/snapshots"
            )  # save image to file 'snapshots' instead of source
